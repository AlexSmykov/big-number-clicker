export enum EUpgradeBuyType {
  ENDLESS = 'ENDLESS',
  WITH_COUNT = 'WITH_COUNT',
  ONE_TIME = 'ONE_TIME',
}

export enum EUpgradeType {
  MONEY = 'MONEY',
  CRYSTAL = 'CRYSTAL',
  RUBY = 'RUBY',
  PRESTIGE = 'PRESTIGE',
}

export enum EUpgradeTier {
  COMMON = 'COMMON',
  UNCOMMON = 'UNCOMMON',
  UNIQUE = 'UNIQUE',
  RARE = 'RARE',
  EPIC = 'EPIC',
  LEGENDARY = 'LEGENDARY',
  MYTHIC = 'MYTHIC',
}

export enum EUpgradeCostGrowth {
  NONE = 'NONE',
  PLUS = 'PLUS',
  PLUS_BY_COUNT = 'PLUS_BY_COUNT',
  MULTIPLY = 'MULTIPLY',
  MULTIPLY_BY_COUNT = 'MULTIPLY_BY_COUNT',
  POWER = 'POWER',
}

export enum EUpgrades {
  // region Endless
  SIMPLE_MULTIPLIER = 'SIMPLE_MULTIPLIER',
  SIMPLE_MULTIPLIER_BOOST = 'SIMPLE_MULTIPLIER_BOOST',
  CRYSTAL_CHANCE = 'CRYSTAL_CHANCE',
  CRYSTAL_MULTIPLIER = 'CRYSTAL_MULTIPLIER',
  LOG_MULTIPLIER_BASE = 'LOG_MULTIPLIER_BASE',
  SIMPLE_MULTIPLIER_POWER = 'SIMPLE_MULTIPLIER_POWER',
  PRESTIGE_MULTIPLIER = 'PRESTIGE_MULTIPLIER',
  CRYSTAL_MULTIPLIER_COEFFICIENT = 'CRYSTAL_MULTIPLIER_COEFFICIENT',
  PRESTIGE_BORDER_DECREASE = 'PRESTIGE_BORDER_DECREASE',
  PERMANENT_SIMPLE_MULTIPLIER = 'PERMANENT_SIMPLE_MULTIPLIER',
  RUBY_CHANCE_INCREASE = 'RUBY_CHANCE_INCREASE',
  // endregion

  // region Countable
  LOG_MULTIPLIER_POWER = 'LOG_MULTIPLIER_POWER',
  CRYSTAL_CHANCE_BY_MONEY = 'CRYSTAL_CHANCE_BY_MONEY',
  START_FLAT_BONUS = 'START_FLAT_BONUS',
  PRESTIGE_POINTS_GAIN = 'PRESTIGE_POINTS_GAIN',
  CRYSTAL_CHANCE_BY_PRESTIGE_POINTS = 'CRYSTAL_CHANCE_BY_PRESTIGE_POINTS',
  // endregion

  // region One time
  UNLOCK_CRYSTALS = 'UNLOCK_CRYSTALS',
  UNLOCK_SETTINGS_AND_ABOUT = 'UNLOCK_SETTINGS_AND_ABOUT',
  UNLOCK_PRESTIGE = 'UNLOCK_PRESTIGE',
  UNLOCK_LOG_MULTIPLIER = 'UNLOCK_LOG_MULTIPLIER',
  UNLOCK_RUBIES = 'UNLOCK_RUBIES',
  UNLOCK_MORE_UPGRADES_1 = 'UNLOCK_MORE_UPGRADES_1',
  CRYSTAL_CHANCE_ON_PRESTIGE = 'CRYSTAL_CHANCE_ON_PRESTIGE',
  MULTIPLY_CRYSTAL_GAIN = 'MULTIPLY_CRYSTAL_GAIN',
  RUBY_BASED_MULTIPLIER = 'RUBY_BASED_MULTIPLIER',
  ANOTHER_MULTIPLIER_1 = 'ANOTHER_MULTIPLIER_1',
  SIMPLE_MULTIPLIER_BUY_BONUS = 'SIMPLE_MULTIPLIER_BUY_BONUS',
  UNLOCK_PERMANENT_SIMPLE_MULTIPLIER = 'UNLOCK_PERMANENT_SIMPLE_MULTIPLIER',
  // endregion
}
